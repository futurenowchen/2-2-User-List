...
const peopleData = JSON.parse(localStorage.getItem('favoritePeople'))
...
function removeFromFavorite(id) {
  ...
  const peopleIndex = peopleData.findIndex((people) => people.id === id)
  if (peopleIndex === -1) return

  //刪除該筆人物資料
  peopleData.splice(peopleIndex, 1)

  //存回 local storage
  localStorage.setItem('favoritePeople', JSON.stringify(peopleData))
